...
# === 选择需要更新的记录：end_time 不为空，且至少有一个峰值或平均速度字段为空 ===
query = """
SELECT id, log 
FROM train_record 
...
for row in rows:
    ...
    try:
        ...
        peak_power = float(np.percentile(power,             99.5))   # 通常取正向峰值
        ...
        for d in TARGET_DISTANCES:
            ...
            if total_time > 0:
                avg_speeds[d] = actual_dist / total_time
            else:
                avg_speeds[d] = None
            
            # 距离不足的提示
            if actual_dist < d * 0.95:
                print(f"ID {record_id} | 前 {d}m 只跑到 {actual_dist:.2f}m → avg={avg_speeds[d]:.4f if avg_speeds[d] else 'None'}")
        
        # =====================================
        # 5. 准备更新参数（None 保持数据库 NULL）
        # =====================================
        params = (
            ...
            avg_speeds[5]   if avg_speeds.get(5)   is not None and not np.isnan(avg_speeds[5])   else None,
            avg_speeds[10]  if avg_speeds.get(10)  is not None and not np.isnan(avg_speeds[10])  else None,
            avg_speeds[15]  if avg_speeds.get(15)  is not None and not np.isnan(avg_speeds[15])  else None,
            avg_speeds[20]  if avg_speeds.get(20)  is not None and not np.isnan(avg_speeds[20])  else None,
            avg_speeds[25]  if avg_speeds.get(25)  is not None and not np.isnan(avg_speeds[25])  else None,
            avg_speeds[30]  if avg_speeds.get(30)  is not None and not np.isnan(avg_speeds[30])  else None,
            record_id
        )
        ...
        for d in TARGET_DISTANCES:
            v = avg_speeds.get(d)
            print(f"  {d:2}m: {v:.4f if v is not None else 'None':<6}", end=" | ")
        print()  # 换行
